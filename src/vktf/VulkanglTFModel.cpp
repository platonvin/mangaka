...
namespace vkglTF
{
	...
	void Texture::fromglTfImage(tinygltf::Image &gltfimage, std::string path, TextureSampler textureSampler, Renderer* renderer)
	{
...
println
			...
			if (renderer->physicalDeviceFeatures.textureCompressionBC) {
				...
				auto formatSupported = renderer->findSupportedFormat({VK_FORMAT_BC7_UNORM_BLOCK, VK_FORMAT_BC3_SRGB_BLOCK}, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_TRANSFER_DST_BIT | VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT);	
...
println
			...
			if (renderer->physicalDeviceFeatures.textureCompressionETC2) {
				auto formatSupported = renderer->findSupportedFormat({VK_FORMAT_ASTC_4x4_SRGB_BLOCK}, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_TRANSFER_DST_BIT | VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT);	
			...
			image.mip_levels = ktxTranscoder.get_levels();
			...
			for (uint32_t i = 0; i < image.mip_levels; i++) {
				ktxTranscoder.get_image_level_info(levelInfos[i], i, 0, 0);
			}

			image.extent.width = levelInfos[0].m_orig_width;
			image.extent.height = levelInfos[0].m_orig_height;
			...
			if (!success) {
				crash("Could not start transcoding for image file " + filename);
			}
			...
			for (uint32_t i = 0; i < image.mip_levels; i++) {
				...
				if (!ktxTranscoder.transcode_image_level(i, 0, 0, bufferPtr, numBlocksOrPixels, targetFormat, 0)) {
					crash("Could not transcode the requested image file " + filename);
				}
...
println
			...
			renderer->createBufferStorages(&stagingBuffer, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, totalBufferSize, true);
...
println
			...
			for (uint32_t i = 0; i < image.mip_levels; i++) {
				...
				vkCmdCopyBufferToImage(copyCmd, stagingBuffer.buffer, image.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &bufferCopyRegion);
...
println
			...
			bufferCopyRegion.imageSubresource.baseArrayLayer = 0;
			bufferCopyRegion.imageSubresource.layerCount = 1;
			bufferCopyRegion.imageExtent.width = image.extent.width;
			bufferCopyRegion.imageExtent.height = image.extent.height;
			bufferCopyRegion.imageExtent.depth = 1;
...
println
			...
			for (uint32_t i = 1; i < image.mip_levels; i++) {
				...
				vkCmdBlitImage(blitCmd, image.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, image.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageBlit, VK_FILTER_LINEAR);
			...
			renderer->endSingleTimeCommands(blitCmd);
		...
		VK_CHECK(vkCreateSampler(renderer->device, &samplerInfo, nullptr, &sampler));
	}
	...
	// Mesh
	Mesh::Mesh(Renderer* _renderer, glm::mat4 matrix) {
		renderer = _renderer;
...
println
	...
	void Model::loadFromFile(std::string filename, Renderer* renderer, float scale)
	{
...
println
		...
		vmaUnmapMemory(renderer->VMAllocator, vertexStaging.alloc);
		vmaUnmapMemory(renderer->VMAllocator, indexStaging.alloc);
		vmaDestroyBuffer(renderer->VMAllocator, vertexStaging.buffer, vertexStaging.alloc);
		vmaDestroyBuffer(renderer->VMAllocator, indexStaging.buffer, indexStaging.alloc);
